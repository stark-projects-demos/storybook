import { readFile } from "node:fs/promises";

import { defineConfig, type Options } from "tsup";
import { raw } from "esbuild-raw-plugin";

import {
  BROWSER_TARGETS,
  NODE_TARGET,
} from "storybook/internal/builder-manager";
import { globalPackages as globalManagerPackages } from "storybook/internal/manager/globals";
import { globalPackages as globalPreviewPackages } from "storybook/internal/preview/globals";

type BundlerConfig = {
  bundler?: {
    exportEntries?: string[];
    nodeEntries?: string[];
    managerEntries?: string[];
    previewEntries?: string[];
    browserIifeEntries?: string[];
  };
};

export default defineConfig(async (options) => {
  // reading the three types of entries from package.json, which has the following structure:
  // {
  //  ...
  //   "bundler": {
  //     "exportEntries": ["./src/index.ts"],
  //     "managerEntries": ["./src/manager.ts"],
  //     "previewEntries": ["./src/preview.ts"]
  //     "nodeEntries": ["./src/preset.ts"]
  //   }
  // }
  const packageJson = (await readFile("./package.json", "utf8").then(
    JSON.parse,
  )) as BundlerConfig;
  const {
    bundler: {
      exportEntries = [],
      managerEntries = [],
      previewEntries = [],
      nodeEntries = [],
      browserIifeEntries = [],
    } = {},
  } = packageJson;

  const commonConfig: Options = {
    splitting: false,
    minify: !options.watch,
    treeshake: true,
    sourcemap: true,
    // keep this line commented until https://github.com/egoist/tsup/issues/1270 is resolved
    // clean: options.watch ? false : true,
    clean: false,
    esbuildPlugins: [raw()],
  };

  const configs: Options[] = [];

  // export entries are entries meant to be manually imported by the user
  // they are not meant to be loaded by the manager or preview
  // they'll be usable in both node and browser environments, depending on which features and modules they depend on
  if (exportEntries.length) {
    configs.push({
      ...commonConfig,
      entry: exportEntries,
      dts: {
        resolve: true,
      },
      format: ["esm", "cjs"],
      platform: "neutral",
      target: NODE_TARGET,
      external: [...globalManagerPackages, ...globalPreviewPackages],
      esbuildPlugins: [raw()],
    });
  }

  // manager entries are entries meant to be loaded into the manager UI
  // they'll have manager-specific packages externalized and they won't be usable in node
  // they won't have types generated for them as they're usually loaded automatically by Storybook
  if (managerEntries.length) {
    configs.push({
      ...commonConfig,
      entry: managerEntries,
      format: ["esm"],
      platform: "browser",
      target: BROWSER_TARGETS,
      external: globalManagerPackages,
      esbuildPlugins: [raw()],
    });
  }

  // preview entries are entries meant to be loaded into the preview iframe
  // they'll have preview-specific packages externalized and they won't be usable in node
  // they'll have types generated for them so they can be imported when setting up Portable Stories
  if (previewEntries.length) {
    configs.push({
      ...commonConfig,
      entry: previewEntries,
      dts: {
        resolve: true,
      },
      format: ["esm", "cjs"],
      platform: "browser",
      target: BROWSER_TARGETS,
      external: globalPreviewPackages,
      esbuildPlugins: [raw()],
    });
  }

  // node entries are entries meant to be used in node-only
  // the CLI entry (src/cli/index.ts) is built as CJS and output as dist/cli.cjs
  if (nodeEntries.length) {
    configs.push({
      ...commonConfig,
      entry: { cli: nodeEntries[0] },
      format: ["cjs"],
      platform: "node",
      target: NODE_TARGET,
      esbuildPlugins: [raw()],
    });
  }

  // browser IIFE entries are injected into headless browser pages at runtime
  // they bundle all dependencies and expose globals for use by the CLI scanner
  if (browserIifeEntries.length) {
    configs.push({
      ...commonConfig,
      entry: { "scanner.browser": browserIifeEntries[0] },
      format: ["iife"],
      platform: "browser",
      target: BROWSER_TARGETS,
      noExternal: [/^@stark-contrast\/.*/],
      outExtension: () => ({ js: ".js" }),
      esbuildPlugins: [raw()],
    });
  }

  return configs;
});
